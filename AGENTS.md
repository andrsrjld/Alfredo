...
- Current lint baseline is not clean: known errors are unused variables in `src/lib/supabase/server.ts` and `src/components/realtime/RealtimeServerStatus.tsx`, plus `no-explicit-any` in `src/app/api/webhook/whatsapp/route.ts` and `src/lib/search.ts`.
...
- Webhook routes: GitLab `src/app/api/webhook/gitlab/route.ts`, WhatsApp `src/app/api/webhook/whatsapp/route.ts`, server cron ping `src/app/api/server-ping/route.ts`.
